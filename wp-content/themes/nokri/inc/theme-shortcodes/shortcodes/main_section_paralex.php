			...
			foreach($rows as $row )
			{
				if( isset( $row['cat'] )  )
				{
					...
					if(isset($want_to_show) && $want_to_show == "yes")
					{
						...
						{
							$cats_html .= '<option value="'.$category->term_id.'" >'.$category->name.'  ('.$custom_count.')' ;
				...
				foreach( $ad_cats as $cat )
				{
					if(isset($want_to_show) && $want_to_show == "yes")
					{
					   ...
					   //sub cat
						...
						{
							$cats_html .= '<option value="'.$cat->term_id.'" >'.$cat->name.'  ('.$custom_count.')' ;
						...
						{
							$cats_html .= '<option value="'.$cat->term_id.'">'.$cat->name.'   ('.$custom_count.')</option>';
						}
					}
					else
					{
			...
			if( count( $rows_hot_cats ) > 0 )
			{
				foreach($rows_hot_cats as $rows_hot_cat )
				{
					if( isset( $rows_hot_cat['hot_cat'] )  )
					{
						...
						continue;
						$hot_cats_html .= '<a href="'.get_the_permalink($nokri['sb_search_page']).'?cat-id='.$get_hot_cat->term_id.'">'.$get_hot_cat->name.'</a>';
					}
				}
			}
		}

		...
		if(isset ($nokri[$key]) && is_array($nokri[$key]) && $nokri[$key] !="")
		{
			...
			if( count((array)$footer_job_taxonomy_links ) > 0) 
			{
				foreach($footer_job_taxonomy_links as $key => $val )
				{
					...
					if($term)
					{
						$fetch_job_taxonomy    .='<li><a href="'.get_the_permalink( $nokri['sb_search_page'] ).'?'.$qstring.'='.$val.'">'.$term->name.'</a></li>';
 ...
 function nokri_static_strings()
 {
   ...
   'get_strings',
    array(
	 ...
	 /* For apply */
	 'apply_without'   => esc_html__( 'Applied and logged in successfully', 'nokri' ),
	 /* For Education */
